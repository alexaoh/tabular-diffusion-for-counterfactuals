...
def take_args():
    ...
    parser = argparse.ArgumentParser(prog = "generate_data_CH.py", 
                                     description = "Generate synthetic data for CH with trees in MCCE.")
...
def main(args):
    ...
    model = random_model()
    mcce = MCCE(dataset, model, seed = args.seed)
    ...
    generated_data = mcce.generate(d1, k = 1) # In this case we generated the same number as CH, since we get no NaNs. 
    ...
    # Save to disk.
    generated_data.to_csv("synthetic_data/CH_from_trees"+str(seed)+".csv")
