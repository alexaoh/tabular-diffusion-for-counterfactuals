...
def take_args():
    ...
    parser = argparse.ArgumentParser(prog = "generate_data_DI.py", 
                                     description = "Generate synthetic data for DI with TVAE.")
...
def main(args):
        ...
        if args.train:
                ...
                with open("pytorch_models/DI_TVAE"+str(seed)+".obj", "wb") as f:
                        pickle.dump(tvae, f)

        if not args.train:
                # Load fitted model from disk.
                with open("pytorch_models/DI_TVAE"+str(seed)+".obj", 'rb')  as f:
                        tvae = pickle.load(f)
        ...
        if args.num_samples is None:
               generated_data = tvae.sample(samples = d1.shape[0]) # Generate DI-size of synthetic data. 
        else:
        ...
        if args.savename is None:
               generated_data.to_csv("synthetic_data/DI_TVAE"+str(seed)+".csv")
        else:
               generated_data.to_csv("synthetic_data/DI_TVAE_"+str(args.savename)+"_"+str(seed)+".csv")
