...
class Encoder(nn.Module):
    def __init__(self, input_size, latent_dim):
        ...
        # When modifying for VAE we need 2*latent_dim here (for mu and log_var).
...
autoencoder = Autoencoder(input_size, latent_dim).to(device)

# Train the autoencoder.
autoencoder, train_losses = train(autoencoder, train_data_loader, epochs = 5)
...
def synthesize(autoencoder):
    ...
    x_n = y_n = 100
    x_points = np.linspace(-10, 5, x_n)
    y_points = np.linspace(-6, 2, y_n)
...
synthetic_data_de.to_csv("synth_ae.csv")
