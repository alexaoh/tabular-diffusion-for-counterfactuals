...
def find_levels(df, categorical_features):
    """Returns a list of levels of features of each of the categorical features in a data frame 'df'."""
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self, model, n):
        ...
        model.eval()
        x_list = {}
        # Sampling needs to be changed such that we can sample noise from the numerical covariates and the original data point from the categorical covariates!
        with torch.no_grad():
    ...
    def loss(self, real_noise, model_pred):
        ...
        return torch.mean((real_noise - model_pred)**2) # We take the mean over all dimensions. Seems correct based on results. 
...
def train(X_train, y_train, X_valid, y_valid, numerical_features, categorical_features, lens_categorical_features, 
    ...
    # Cross-entropy loss for the categorical variables. 
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs,_) in enumerate(train_loader):
            ...
            # Here we want the model to predict the noise for the numerical covariates but the original input for the categorical features. 

            # Since we are dealing with both numerical and categorical features, we need to split the output (and the noise) according to the different features.
            # We assume that the numerical features are first, then the rest are categorical.
            preds_numerical = predicted_noise[:,:len(numerical_features)]
            preds_categorical = torch.split(predicted_noise[:,len(numerical_features):], lens_categorical_features, dim = 1)

            # Gaussian diffusion uses MSE loss. 
            loss_MSE = diffusion.loss(noise[:,:len(numerical_features)], preds_numerical)
...
# We try so run the entire data set through the Gaussian diffusion model:
# Quantile transformed numerical covariates and one-hot encoded categorical covariates. 
...
training_losses, validation_losses = train(X_train, y_train, X_test, y_test, numerical_features, 
                        categorical_features, lens_categorical_features, device, T = 100, 
                        schedule = "linear", batch_size = 4096, num_epochs = 150, 
...
def evaluate(n, generate = True, plot_corr = True, save_figs = True): 
    ...
    with torch.no_grad():
        ...
        synthetic_descaled = Adult.descale(synthetic_samples)
        print(synthetic_descaled.shape)

        synthetic_backtransformed = Adult.decode(synthetic_descaled)
        ...
        def visualize_numerical_data(synthetic_data, real_data):
            """Plot histograms over synthetic data against real training data for numerical features."""
            synthetic_data = synthetic_data[numerical_features]
            real_data = real_data[numerical_features]
        ...
        visualize_numerical_data(synthetic_samples, X_train)
        plt.show()

        # Visualize again after descaling.
        visualize_numerical_data(synthetic_descaled, X_train)
        if save_figs:
            plt.savefig("backtransformed_numerical_synthetic_gaussian_entire_data_MSE.pdf")
        #plt.show()
        # Check that these two are equal. They look the same!
        visualize_numerical_data(synthetic_backtransformed, X_train)
        plt.show()

        visualize_categorical_data(synthetic_backtransformed, Adult.get_training_data()[0])
        if save_figs:
            plt.savefig("backtransformed_categorical_synthetic_gaussian_entire_data_MSE.pdf")
        plt.show()

        def plot_correlation(synth, true, descaled = True):
            synth = synth[numerical_features]
            true = true[numerical_features]
            ...
            plt.tight_layout()
            if save_figs:
                plt.savefig("backtransformed_absolute_difference_correlation_gaussian_entire_data_MSE.pdf")
            plt.show()
        ...
        if plot_corr:
            ...
            print("Descaled data:")
            plot_correlation(synthetic_backtransformed, Adult.get_training_data()[0])

        print(f"Backtransformed synthetic: {synthetic_backtransformed.describe()}")
        print(f"Backtransformed training data: {Adult.decode(Adult.descale(X_train)).describe()}")
        ...
        def show_difference_in_describe(synthetic_samples):
            ...
            desc_synth = synthetic_samples[numerical_features].describe()
        ...
        show_difference_in_describe(synthetic_backtransformed)
...
#evaluate(X_train.shape[0], generate = False, plot_corr=True, save_figs=False)
