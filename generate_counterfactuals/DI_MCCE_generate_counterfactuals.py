...
def take_args():
    ...
    parser = argparse.ArgumentParser(prog = "DI_MCCE_generate_counterfactuals.py", 
                                     description = "Generate counterfactuals for factuals in DI with MCCE.")
...
def main(args):
    ...
    categorical_encoded = categorical_features
    ...
    if args.generate:
        ...
        cfs.to_csv("counterfactuals/DI_MCCE_Dh_K"+str(args.K)+"_"+str(seed)+".csv")
    else: 
        # Load the generated possible counterfactuals, Dh, from disk. 
        cfs = pd.read_csv("counterfactuals/DI_MCCE_Dh_K"+str(args.K)+"_"+str(seed)+".csv", index_col = 0)
    ...
    cfs.to_csv("counterfactuals/DI_MCCE_final_K"+str(args.K)+"_"+str(seed)+".csv")
