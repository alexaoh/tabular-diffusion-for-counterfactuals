...
def take_args():
    ...
    parser = argparse.ArgumentParser(prog = "DI_TabDDPM_generate_counterfactuals.py", 
                                     description = "Generate counterfactuals for factuals in DI with TabDDPM.")
...
def main(args):
    ...
    # python train_diffusion.py -s 1234 -d DI -t True -g True -T 1000 -e 200 -b 256 --mlp-blocks 256 1024 1024 1024 1024 256 --dropout-ps 0 0 0 0 0 0 --early-stop-tolerance "None" --num-samples 10000 --savename "TabDDPM_K10000_"
        
    # Load the generated possible counterfactuals, Dh, from disk. 
    extra = ""
    if args.joint:
        extra = "_joint"
    cfs = pd.read_csv("synthetic_data/DI_TabDDPM_K"+str(args.K)+extra+"_"+str(args.seed)+".csv", index_col = 0)
    ...
    filename = "counterfactuals/DI_TabDDPM_final_K"+str(args.K)+extra+"_"+str(seed)+".csv"
    cfs.to_csv(filename)
