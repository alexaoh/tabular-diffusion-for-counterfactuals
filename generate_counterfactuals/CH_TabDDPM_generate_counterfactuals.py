...
def main(args):
    ...
    if args.joint:
        extra = "_joint"
    cfs = pd.read_csv("synthetic_data/CH_TabDDPM_K"+str(args.K)+extra+"_"+str(args.seed)+".csv", index_col = 0)
    ...
    filename = "counterfactuals/CH_TabDDPM_final_K"+str(args.K)+extra+"_"+str(seed)+".csv"
    cfs.to_csv(filename)
