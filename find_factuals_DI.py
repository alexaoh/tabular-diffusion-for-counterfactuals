...
def main(train, save_factuals):
    ...
    if train:
        ...
        model.save_model("predictors/cat_boost_DI.dump")

    if not train:
        model = ctb.CatBoostClassifier()
        model.load_model("predictors/cat_boost_DI.dump")
    ...
    factuals = negatively_predicted_individuals.sample(n = 10, random_state = seed)
    
    # Save the factuals.
    if save_factuals:
        factuals.to_csv("factuals/factuals_DI_catboost.csv")
